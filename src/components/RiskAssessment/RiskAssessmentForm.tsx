...
import { AssessmentData, FormStep, CategoryDetail } from './types';
...
const getTitleAndDescription = (step: FormStep) => {
  switch (step) {
    case 'contact':
      return {
    ...
    case 'results':
      return {
        ...
        description: "Here's what we found based on your responses"
      };
  }
};

const steps: FormStep[] = ['contact', 'provider', 'profile', 'security', 'compliance'];
  ...
  const renderComplianceQuestions = () => (
    ...
    >
      ...
      <Select onValueChange={(value) => handleInputChange('itIssues', value)}>
        <SelectTrigger>
          <SelectValue placeholder="How frequently do you face IT problems? (e.g., crashes, slowness)" />
        </SelectTrigger>
        <SelectContent>
